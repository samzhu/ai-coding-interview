...
import type { InterviewResponse, InterviewStatus } from "@/types/interview";
...
export function InterviewListTable({ interviews }: InterviewListTableProps) {
  ...
  async function handleCopyInvite(interviewId: string) {
    ...
    try {
      ...
      await navigator.clipboard.writeText(`${appUrl}/invite/${invitation.token}`);
    } catch {
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {interview.status === "IN_PROGRESS" && (
                        <Button variant="secondary" size="sm" asChild>
                          <Link href={`/admin/interviews/${interview.id}/monitor`}>
                            監控
                          </Link>
                        </Button>
                      )}
